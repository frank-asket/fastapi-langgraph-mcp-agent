  ...
  return (
    <aside className="flex w-56 shrink-0 flex-col border-r border-sc-line bg-sc-elev shadow-[4px_0_24px_rgba(0,0,0,0.2)] lg:w-64">
      ...
      <div className="border-t border-sc-line p-3">
        {hasClerk ? (
          <div className="flex items-center gap-2 rounded-xl border border-sc-line bg-sc-bg px-2 py-2">
            <SignedIn>
              <UserButton afterSignOutUrl="/studio" appearance={{ elements: { avatarBox: "h-9 w-9" } }} />
              <span className="truncate text-xs text-sc-mist">Account</span>
            </SignedIn>
            <SignedOut>
          ...
          <p className="text-center text-[0.65rem] text-[#6a756d]">Configure Clerk for protected routes</p>
        )}
      </div>
    </aside>
  );
