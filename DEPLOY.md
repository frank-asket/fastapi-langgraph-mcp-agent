...
2. **Variables** (mirror root `.env`): at minimum `OPENAI_API_KEY`, `SESSION_SECRET`, `SESSION_COOKIE_SECURE=true`, `PUBLIC_BASE_URL` (your **canonical** API URL, e.g. `https://coach.klingbo.com` with custom domain, not only the default `*.up.railway.app` name—so redirects and MCP URLs stay consistent), `STUDY_COACH_FRONTEND_URL` (e.g. `https://study.klingbo.com`), and `CORS_ORIGINS` listing **every** frontend origin the browser uses (Vercel preview URL, custom domain, etc.). When `CORS_ORIGINS` is set (non-empty), the API **also merges** `STUDY_COACH_FRONTEND_URL` into the allowlist so the canonical UI origin is covered if you omitted it from `CORS_ORIGINS`. An **empty** `CORS_ORIGINS` disables CORS entirely (no merge). Clerk/SendGrid keys as needed. The Docker image defaults SQLite paths under **`/data`** — add a **Volume** in Railway mounted at **`/data`** so data survives redeploys.
